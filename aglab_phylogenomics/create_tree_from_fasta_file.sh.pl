...
if ( scalar (@ARGV) == 0 ) {
	my @tmp = split ( /\//, $0 );
	die "USAGE: $tmp[0] <input fasta file> [<outgroup>]\n\n";
}
...
if ( $OUTGROUP ) {
	system ( "raxmlHPC-PTHREADS-SSE3 -f a -x 12345 -p 12345 -N 100 -T 2 -m PROTGAMMAJTT -s $in_file.renamed.aln.trimmed.phy -w $PWD -n $in_file.renamed.aln.trimmed.phy.tre -o $OUTGROUP" );
}
else {
	system ( "raxmlHPC-PTHREADS-SSE3 -f a -x 12345 -p 12345 -N 100 -T 2 -m PROTGAMMAJTT -s $in_file.renamed.aln.trimmed.phy -w $PWD -n $in_file.renamed.aln.trimmed.phy.tre" );
}
print STDERR "Done!\n\n";
...
system ( "raxmlHPC-SSE3 -L MRE -z RAxML_bootstrap.$in_file.renamed.aln.trimmed.phy.tre -m GTRCAT -n T1" );
print STDERR "Done!\n";
